...
type RestOf<T extends IComment | NewComment> = Omit<T, 'timestamp'> & {
...
export class CommentService {
  ...
  protected convertDateFromClient<T extends IComment | NewComment | PartialUpdateComment>(comment: T): RestOf<T> {
    return {
      ...comment,
      timestamp: comment.timestamp?.toJSON() ?? null,
    };
  }
  ...
  protected convertDateFromServer(restComment: RestComment): IComment {
    return {
      ...restComment,
      timestamp: restComment.timestamp ? dayjs(restComment.timestamp) : undefined,
    };
  }
