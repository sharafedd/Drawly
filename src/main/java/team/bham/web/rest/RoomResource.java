...
/**
 * REST controller for managing {@link team.bham.domain.Room}.
 */
@RestController
@RequestMapping("/api")
@Transactional
public class RoomResource {
    ...
    public ResponseEntity<Room> partialUpdateRoom(@PathVariable(value = "id", required = false) final Long id, @RequestBody Room room)
        ...
        Optional<Room> result = roomRepository
            .findById(room.getId())
            .map(existingRoom -> {
                if (room.getNoOfRounds() != null) {
                    existingRoom.setNoOfRounds(room.getNoOfRounds());
                }
                if (room.getNoOfPlayers() != null) {
                    existingRoom.setNoOfPlayers(room.getNoOfPlayers());
                }
                ...
                if (room.getTheme() != null) {
                    existingRoom.setTheme(room.getTheme());
                }
